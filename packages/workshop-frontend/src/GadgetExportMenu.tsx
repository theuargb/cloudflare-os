...
export default function GadgetExportMenu({ gadget, gadgetTitle, chatId }: Props) {
  ...
  const download = async (format: GadgetExportFormat) => {
    ...
    try {
      ...
      toasts.add({ title: `Не вдалося експортувати ${format.label}`, variant: 'error' })
    } finally {
      setExportingId(null)
    }
  }
      ...
      <span className="relative inline-flex">
        <DropdownMenu onOpenChange={handleOpenChange}>
          <DropdownMenu.Trigger
            render={(
              <WorkshopIconButton
                aria-label="Експортувати гаджет"
          ...
          <DropdownMenu.Content className="themed-floating-shadow !z-[1100] !min-w-[144px] rounded-lg border border-kumo-line bg-kumo-base p-1">
            {loading ? (
              <div role="status" aria-label="Завантаження форматів експорту" className="space-y-1 py-0.5">
                {['w-20', 'w-14'].map(width => (
              ...
              <div className="px-2.5 py-2 text-[12px] leading-4 text-kumo-subtle">
                <p>Не вдалося завантажити формати експорту.</p>
                <button
                  type="button"
                  onClick={loadFormats}
              ...
              <p className="px-2.5 py-2 text-[12px] leading-4 text-kumo-subtle">
                Цей гаджет не підтримує експорт.
              </p>
            ) : formats?.map(format => (
              <DropdownMenu.Item
                key={format.id}
