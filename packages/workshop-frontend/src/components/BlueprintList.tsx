...
export default function BlueprintList() {
    ...
    Promise.all([authenticatedApi.listOwnBlueprints(), authenticatedApi.listLibraryBlueprints()])
      .then(([own, library]) => {
        ...
        const ensure = (id: string): BlueprintItem => {
          let it = map.get(id)
          if (!it) {
            it = { id, title: 'Шаблон без назви', description: '', recency: 0, pinned: false, inLibrary: false, isOwn: false }
            map.set(id, it)
          }
          return it
        }
  ...
  const handleBlueprintSelected = useCallback(async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toasts.add({ title: 'Шаблон завантажено', variant: 'success' })
      load()
    } catch (err) {
      console.error('Failed to upload blueprint:', err)
      toasts.add({ title: 'Не вдалося завантажити шаблон', variant: 'error' })
    } finally {
      setUploading(false)
    }
  ...
  const handleRemoveFromLibrary = async (item: BlueprintItem) => {
    try {
      ...
      toasts.add({ title: 'Вилучено з бібліотеки', variant: 'success' })
    } catch (err) {
      console.error('Failed to remove blueprint from library:', err)
      toasts.add({ title: 'Не вдалося вилучити шаблон', variant: 'error' })
    }
  }
        ...
        <div className="mb-4 flex flex-col gap-2 px-3 sm:flex-row sm:items-center">
          ...
          <div className="grid w-full grid-cols-2 gap-2 sm:w-auto sm:shrink-0">
            ...
            <button
              ...
              disabled={uploading}
              title="Завантажити архів .gadget"
      ...
      <div className="chat-panel flex min-h-0 flex-1 flex-col gap-0.5 overflow-y-auto pt-1">
        {loading ? (
          ...
          <div className="py-12 text-center text-sm">
            <p className="text-kumo-danger">Під час завантаження шаблонів сталася помилка.</p>
            <button type="button" onClick={load} className="mt-1 text-kumo-brand underline">Спробувати ще раз</button>
          </div>
        ) : filtered.length === 0 ? (
          search ? (
            <div className="py-12 text-center text-sm text-kumo-inactive">Шаблонів не знайдено</div>
          ) : (
            <div className="flex flex-col items-center gap-3 px-3 py-16 text-center">
              ...
              <div>
                <p className="text-sm font-medium text-kumo-default">Шаблонів поки немає</p>
                <p className="mt-1 text-[13px] leading-[18px] text-kumo-subtle">
                  Опублікуйте робочий простір як шаблон або додайте шаблон з огляду.
                </p>
              </div>
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  <UploadSimple size={14} weight="bold" />
                  {uploading ? 'Завантаження…' : 'Завантажити .gadget'}
                </button>
              </div>
            </div>
          )
