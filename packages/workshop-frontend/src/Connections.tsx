...
export default function Connections({ overseer, gadget, chatId, authenticatedApi, onConnectionsChange, isVisible, onHasGatekeepersChange }: ConnectionsProps) {
  ...
  const loadGatekeepers = async () => {
    try {
      ...
      toasts.add({ title: 'Не вдалося завантажити підключення', variant: 'error' })
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDeleteHookConfirm = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Не вдалося видалити тригер', variant: 'error' })
    } finally {
  ...
  const handleEditSave = async (name: string) => {
    ...
    if (!newName) {
      toasts.add({ title: 'Назва прив’язування не може бути порожньою', variant: 'error' })
      return
    }
    if (newName === name) {
    ...
    try {
      ...
      toasts.add({ title: 'Не вдалося оновити назву прив’язування', variant: 'error' })
    } finally {
      setEditingBinding(null)
    }
  }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Не вдалося вилучити підключення', variant: 'error' })
    } finally {
  ...
  return (
    <div className="h-full overflow-auto bg-kumo-base">
      <div className="mx-auto flex min-h-full w-full max-w-5xl flex-col px-4 py-5 sm:px-6">
        <section>
          <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-subtle">
                Зовнішні ресурси, якими може користуватися цей гаджет.
              </p>
            </div>
            <WorkshopButton
              tone="primary"
          ...
          {loading ? (
            ...
            <EmptyState
              title="Немає підключених ресурсів"
              description="Підключіть Google Docs, GitHub, Google Sheets та інші сервіси, щоб гаджет міг безпечно використовувати зовнішні дані."
              actionLabel="Підключити ресурс"
                    ...
                    {isDeleting ? (
                      <div className="flex flex-wrap items-center gap-3">
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-danger">
                            Видалити «{gk.resourceTitle}»?
                          </p>
                          <p className="truncate text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                            Це прив’язування <span className="font-mono">{gk.name}</span> буде вилучено з цього гаджета.
                          </p>
                        </div>
                        <WorkshopButton
                          tone="danger"
                      ...
                      <div className="flex items-center gap-2">
                        <WorkshopInput
                          ...
                          placeholder="Назва прив’язування"
                          aria-label="Назва прив’язування"
                          autoFocus
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="flex items-center gap-2 truncate text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-default">
                            ...
                            {isPending && (
                              <Tooltip content="Додано в цьому чаті; підключення збережеться після прийняття змін" asChild>
                          ...
                          <p className="mt-0.5 truncate text-[11px] leading-4 tracking-[-0.1px] text-kumo-inactive">
                            У коді використовується назва: <span className="font-mono text-kumo-subtle">{gk.name}</span>
                          </p>
                        </div>
                        <div className="ml-auto flex shrink-0 items-center gap-1">
                          <Tooltip content="Редагувати назву, що використовується в коді" asChild>
                            <WorkshopIconButton
                              onClick={() => handleEditStart(gk.name)}
                              aria-label="Змінити назву в коді"
                            >
                              <Pencil size={14} />
                            </WorkshopIconButton>
                          </Tooltip>
                          {!isPending && (
                            <Tooltip content="Редагувати налаштування шаблону" asChild>
                              <WorkshopIconButton
                                onClick={() => setAnnotationTarget(gk)}
                                aria-label="Змінити налаштування шаблону"
                              >
                                <Blueprint size={14} />
                              </WorkshopIconButton>
                            </Tooltip>
                          )}
                          <Tooltip content="Видалити підключення" asChild>
                            <WorkshopIconButton
                              danger
        ...
        {!loading && hooks.length > 0 && (
          <section className="mt-8">
            <div className="mb-3">
              ...
              <p className="mt-1 text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-subtle">
                Зворотні виклики, які запускають цей гаджет у відповідь на події підключених ресурсів.
              </p>
            </div>
                    ...
                    {isDeleting ? (
                      <div className="flex flex-wrap items-center gap-3">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="truncate text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                            Цей тригер буде видалено назавжди. Майбутні події більше не надходитимуть.
                          </p>
                        </div>
                        <WorkshopButton
                          tone="danger"
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="ml-auto flex shrink-0 items-center gap-2">
                          ...
                          />
                          <Tooltip content="Видалити тригер" asChild>
                            <WorkshopIconButton
                              danger
          ...
          try {
            ...
            toasts.add({
              title: chatId === undefined
                ...
                : "Підключення створено — прийміть зміни чату, щоб зберегти його",
              variant: 'success',
            })
            await loadGatekeepers()
            onConnectionsChange?.()
      ...
      <BlueprintAnnotationModal
        ...
        onSaved={() => {
          toasts.add({ title: 'Налаштування шаблону збережено.', variant: 'success' })
      ...
      try {
        ...
        if (!cancelled) {
          if (loaded) {
            ...
            setLoadError('Підключення не знайдено.')
          }
        }
      } catch (err: any) {
        if (!cancelled) {
        ...
        <Dialog className="responsive-dialog !z-[1000] !w-[min(480px,calc(100vw-32px))] overflow-hidden bg-kumo-base p-0" size="lg">
          <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-4 py-4 sm:px-5">
            <div className="min-w-0">
              ...
              <Dialog.Description className="mt-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                Як це підключення відображатиметься у шаблонах.
              </Dialog.Description>
            </div>
            <Dialog.Close
              render={(props) => (
          ...
          <div className="space-y-4 px-4 py-4 sm:px-5">
            {loadError ? (
              ...
              <div className="py-2 text-center text-[13px] text-kumo-subtle">Завантаження…</div>
            ) : (
              <>
                <BlueprintBindingCard
                  data={data}
